...
class GroupShoutPostEvent extends EventEmitter {
     ...
     * @example
     * var onShout = new group.onShout(groupId, {interval:2000})
    ...
    /**
     * 
     * @param {GroupClass} groupId The group id
    ...
    init(groupId) {
        ...
        this.interval = setInterval(() => {
            ...
            fetch(`https://groups.roblox.com/v1/groups/${groupId}`).then(res=>{
                var body = JSON.parse(res.body);
                var theShout = body.shout;
                if (!theShout || !theShout.body) return;
                if (!global.Shouts[groupId]) {
                    global.Shouts[groupId] = new groupClass.GroupShout(theShout);
                } else if (global.Shouts[groupId].body!=theShout.body) {
                    if (!global.Shouts[groupId].body) return;

                    global.Shouts[groupId] = new groupClass.GroupShout(theShout) //new shout class here
                    //this.emit('wallPost', new groupClass.GroupShout(theShout));
                    this.emit('shout', new groupClass.GroupShout(theShout))
                }
            })
        }, this.intervalWait);
